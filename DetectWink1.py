...
def detect(frame, faceCascade, eyesCascade): # frame is the image 
    ...
    # gray_frame = cv2.medianBlur(gray_frame, 5) # median smoothing
    ...
    for f in faces:
        ...
        if detectWink(frame, (x, y), faceROI, eyesCascade): # calls another function to detect wink in the ROI, returns boolean
            detected += 1
            cv2.rectangle(frame, (x,y), (x+w,y+h), (255, 0, 0), 2) # draw a blue rectangle of width 2 on the ROI if wink is detected
        else:
            cv2.rectangle(frame, (x,y), (x+w,y+h), (0, 255, 0), 2) # draw a green rectangle of width 2 on the ROI if wink is not detected
    return detected
...
def run_on_folder(cascade1, cascade2, folder):
    ...
    for f in files: # goes through each image in the folder
        ...
        if type(img) is np.ndarray:
            ...
            totalCount += lCnt # each image can have more than one person, so lCnt >= 0
            if windowName != None:
